...
def get_last_model_recall(ml_client):
    ...
    recall_value = float(model.properties.get('Recall', 0.0))

    print(f"Registered insurance cross-selling model version {latest_model_version} "
          f"has performance Recall Value: {recall_value}")
    return recall_value
...
def main():
    ...
    # Get the racall value of newly trained model from the training step
    ...
    if retrained_model_recall_value >= deployed_model_recall:
        ...
        # If criteria is not met, do not register the model in workspace and deploy to batch endpoint
